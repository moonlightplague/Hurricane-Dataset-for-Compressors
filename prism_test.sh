...
BINARY=/home/moonlightplague/projects/test_branch/PRISM/build/prism
PRISM=true
PRISMTEST=true
PRISMTESTAT=false
PRISMTESTFAT=false
EBSTR=("1e-1" "1e-3" "1e-5")
...
for FILENAME in *.f32; do
    ...
    for EB in "${EBSTR[@]}"; do
        ...
        if $PRISMTEST; then
            echo "===========================================" >> "${OUTPUT_FILE}"
            echo "Running prism test mode without auto tuning" >> "${OUTPUT_FILE}"
            echo "===========================================" >> "${OUTPUT_FILE}"
        ...
        if $PRISMTESTAT; then
            echo "========================================" >> "${OUTPUT_FILE}"
            echo "Running prism test mode with auto tuning" >> "${OUTPUT_FILE}"
            echo "========================================" >> "${OUTPUT_FILE}"
        ...
        if $PRISMTESTFAT; then
            echo "===================================================" >> "${OUTPUT_FILE}"
            echo "Running prism test mode with new method auto tuning" >> "${OUTPUT_FILE}"
            echo "===================================================" >> "${OUTPUT_FILE}"
